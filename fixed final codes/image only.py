...
def create_model(input_shape_img):
    ...
    concatenated = Concatenate()([flattened_img])
...
def train_and_evaluate_model(images, labels):
    ...
    for train_index, test_index in kfold.split(images):
        ...
        model = create_model(input_shape_img,)
        model.summary()
        history = model.fit(
            [x_train_img,],
            y_train,
        ...
        # Evaluate the model
        _, accuracy = model.evaluate([x_test_img], y_test)
...
def main():
    ...
    images, labels = load_data(gif_folder, verhoogdedruk, verlaagdedruk)
    assert images.shape[0]   == len(labels), "Data shapes do not match."
